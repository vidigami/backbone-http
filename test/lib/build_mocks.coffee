...
createMockModel = (mock_model_types, model_type) ->
  return mock_model_type if mock_model_type = mock_model_types[model_type.name]
    ...
    do (key, relation) ->
      schema[key] = ->
        ...
        if relation.join_table
          ...
          mock_model_types[mock_join_table.name] = mock_join_table
  ...
  mock_model_types[model_type.name] = MockModel
